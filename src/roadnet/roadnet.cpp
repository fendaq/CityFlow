...
namespace CitySimulator {
    ...
    Point Drivable::getPointByDistance(double dis) const {
        return CitySimulator::getPointByDistance(points, dis);
    }
    ...
    std::vector<LaneLink *> Lane::getLaneLinksToRoad(Road *road) const {
        ...
        for (auto &laneLink : laneLinks) {
            if (laneLink->getEndLane()->getBelongRoad() == road) ret.push_back(laneLink);
        }
        return ret;
    }
...
FOUND:;
    ...
    bool Cross::canPass(const Vehicle *vehicle, LaneLink *laneLink, double distanceToLaneLinkStart) const {
        ...
        if (vehicle->getId() == "flow_3726_0" && foeVehicle->getId() == "flow_22894_0") {
            int xxx = 1;
        }
    ...
    std::vector<const Lane *> RoadNet::getLanes() const {
        std::vector<const Lane *> ret;
        for (auto &road:getRoads())
            for (auto &lane:road.getLanes()) {
                ret.emplace_back(&lane);
            }
        return ret;
    }

    std::vector<Lane *> RoadNet::getLanes() {
        std::vector<Lane *> ret;
        for (auto &road:getRoads())
            for (auto &lane:road.getLanes()) {
                ret.emplace_back(&lane);
    ...
    std::vector<const LaneLink *> RoadNet::getLaneLinks() const {
        std::vector<const LaneLink *> ret;
        for (auto &intersection:getIntersections())
            for (auto &roadLink:intersection.getRoadLinks()) {
                for (auto &laneLink : roadLink.getLaneLinks())
                    ret.emplace_back(&laneLink);
            }
        return ret;
    }

    std::vector<LaneLink *> RoadNet::getLaneLinks() {
        std::vector<LaneLink *> ret;
        for (auto &intersection:getIntersections())
            for (auto &roadLink:intersection.getRoadLinks()) {
                for (auto &laneLink : roadLink.getLaneLinks())
